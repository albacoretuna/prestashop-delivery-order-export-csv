...
SELECT SQL_CALC_FOUND_ROWS
    ' ' AS `ردیف`,
    a.`id_order` AS `شماره ارجاع`,
    'لباس' AS `نوع مرسوله`,
    CONCAT(c.`firstname`, ' ', c.`lastname`) AS `نام گیرنده`,
    CONCAT(states.name,
            '-',
            address.`city`,
            '-',
            address.`address1`) AS `آدرس گیرنده`,
    address.`phone_mobile` AS `شماره همراه گیرنده`,
    address.`phone` AS `تلفن گیرنده`,
    CONCAT(PYEAR(a.`date_add`),
            '/',
            PMONTH(a.`date_add`),
            '/',
            PDAY(a.`date_add`)) AS `تاریخ دریافت سفارش`,
    0 AS `وجه کالا`,
    0 AS `هزینه حمل`,
    address.postcode AS `کد پستی`
FROM
    `ps_orders` a
        LEFT JOIN
    `ps_customer` c ON (c.`id_customer` = a.`id_customer`)
        LEFT JOIN
    `ps_address` address ON address.id_address = a.id_address_delivery
        LEFT JOIN
    `ps_state` states ON states.id_state = address.id_state
        LEFT JOIN
    `ps_order_state` os ON (os.`id_order_state` = a.`current_state`)
	    LEFT JOIN
	`ps_order_detail` order_detail ON (order_detail.id_order = a.`id_order`)
WHERE
    1
GROUP BY a.`id_order`
